...
def predict_on_dst_given_source_model(model, data_src, denoising_seed):
  ...
  for data_dst in DATA_DST:
    ...
    df_freq = pd.read_csv(f'./types_freq/{data_dst}.csv')
    types_to_keep = list(df_freq[df_freq['freq_test'] > FREQ_TEST_GT]['type'].values)

    filtered_metrics_per_type = {m:v.item() for m, v in metrics_per_type.items() if 'f1' not in m and m.split('/')[0].replace('test_type_all_', '/').replace('-', '/') in types_to_keep}

    precisions = []
    recalls = []

    for m, v in filtered_metrics_per_type.items():
      if 'precision' in m:
        precisions.append(v)
      else:
        recalls.append(v)
      
    macro_p_on_types = np.mean(precisions)
    macro_r_on_types = np.mean(recalls)
    macro_f1_on_types = compute_f1(macro_p_on_types, macro_r_on_types)

    metrics_per_types_GT_K = {'metrics_per_types_GT_K/precision': macro_p_on_types,
                              'metrics_per_types_GT_K/recall': macro_r_on_types,
                              'metrics_per_types_GT_K/f1' : macro_f1_on_types
                              }
    ...
    dict_for_pandas = update_with_metrics(dict_for_pandas, metrics_per_types_GT_K)
