...
while getopts "ds:i:k:S:T:K:X:" opt; do
  case $opt in
...
COMPOSED_NAME=$TGT2SRC$KB_MODE

# prepare model yaml
MODEL_CONFIG_TEMPLATE='configs/model/kenn_bert.yaml'
NEW_MODEL_CONFIG_PATH='configs/model/generated_config_files/'$SRC2TGT'_kenn_'$KENN_SETUP$KB_MODE'_bert.yaml'
...
sed -i 's/TGT2SRC.txt/'$COMPOSED_NAME'.txt/g' $NEW_MODEL_CONFIG_PATH
sed -i 's/KENN_SETUP/'$KENN_SETUP'/g' $NEW_MODEL_CONFIG_PATH
sed -i 's/TGT/'$DATA_TGT'/g' $NEW_MODEL_CONFIG_PATH
...
--trainer.callbacks.filename=$DATA_SRC'_kenn_'$KENN_SETUP$KB_MODE'_adapter_bert_ms' \
