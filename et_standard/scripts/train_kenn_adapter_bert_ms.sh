...
NEW_MODEL_CONFIG_PATH='configs/model/generated_config_files/'$SRC2TGT'_kenn_bert.yaml'
cp $MODEL_CONFIG_TEMPLATE $NEW_MODEL_CONFIG_PATH
sed -i 's/TGT2SRC.json/'$TGT2SRC'.json/g' $NEW_MODEL_CONFIG_PATH
sed -i 's/TGT2SRC.txt/'$TGT2SRC'.txt/g' $NEW_MODEL_CONFIG_PATH
...
--trainer.callbacks.filename=$DATA_SRC'_kenn_adapter_bert_ms' \
...
--logger.name=$DATA_SRC'_kenn_adapter_bert_ms'